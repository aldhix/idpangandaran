...
@section('breadcrumb')
<li class="breadcrumb-item"><a href="{{route('admin.post',['id'=>$domain->id])}}">Post {{$domain->domain}}</a></li>
...
var app = new Vue({
  ...
  data: {
  	...
  	loader : false,
  	title : '{{$post->title}}',
  	kategori : {{$post->id_categorie}},
  	code_time : {{$post->code_time}},
  	button : 'update',
  	idpost : {{$post->id}},
		...
		simpan : function (type_save){
			...
			axios.post(reqUrl, {
				...
				if(res.data.status == 1){
					...
					var urlEdit = 
					"{{route('admin.post.edit', ['id'=>':id','iddomain'=>$domain->id])}}";
					urlEdit = urlEdit.replace(':id',this.idpost);
					var stateObj = { foo: "bar" };
					history.pushState(stateObj, "ID Pangandaran Admin | Edit Post", urlEdit);
